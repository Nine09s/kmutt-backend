...
def get_ai_response(context, question, groq_client):
    system_prompt = '''
        ...
        1. **ใช้ข้อมูลจาก context เป็นหลัก**: ตอบตาม chunks จาก PDF ที่ retrieve มาเท่านั้น ห้ามเดาเอง
        2. **ถ้าไม่แน่ใจ:** ให้ตอบว่า "ขออภัยครับ ข้อมูลไม่ชัดเจน แนะนำให้ติดต่อสำนักงานทะเบียนโดยตรง" (อย่าแต่งเรื่องเอง)
        ...
        **ถ้าผู้ใช้ถามเรื่อง "กค.18", "RO.08" หรือ "คืนเงิน":**
        ...
        ✨ ฟีเจอร์สำคัญ (Auto-Fill):
        หากผู้ใช้ต้องการทำเรื่องอะไร ให้สรุปข้อมูลใส่ JSON ท้ายตอบเสมอ:
        - "form_id": รหัสฟอร์ม (เช่น RO.16)
        - "draft_reason": **ช่วยเขียนเหตุผลแทนผู้ใช้ให้เป็นภาษาทางการ** (เช่น ผู้ใช้บอก "ป่วยเป็นไข้" -> ร่างว่า "เนื่องจากข้าพเจ้ามีอาการไข้สูงและอ่อนเพลีย จึงไม่สามารถเข้าเรียนได้...")
        - "draft_subject": หัวข้อเรื่อง (ถ้ามี)
        ...
        [[FORM_DATA: {{
            ...
            "department": "สาขาวิชา... เช่น วิศวกรรมคอมพิวเตอร์", 
